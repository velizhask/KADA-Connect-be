...
Additional Supabase auth variables (required for authentication integration):

```bash
# Public anon key used for server-side token validation (safe for server-side use)
SUPABASE_ANON_KEY=your-anon-key

# Service role key (sensitive) - only required for server-side admin operations
SUPABASE_SERVICE_KEY=your-service-role-key
```
...
The application uses Supabase Auth. The backend exposes simple auth endpoints under `/api/auth`.

- `POST /api/auth/register` — Register a new account (student | company | admin)
- `POST /api/auth/login` — Authenticate and receive session data
- `POST /api/auth/logout` — Invalidate session (requires Authorization header)
...
curl -X POST http://localhost:3001/api/auth/register \
   ...
   -d '{"email":"alice@example.com","fullName":"Alice Example","password":"Pa$$w0rd","role":"student"}'
```
...
```json
{
  ...
  "data": { "id": "<uuid>", "email": "alice@example.com", "role": "student" },
  "session": {
    /* optional session object returned by Supabase */
  }
}
```
...
- The server validates tokens using the Supabase Auth APIs. Keep `SUPABASE_ANON_KEY` and `SUPABASE_URL` configured in your environment.
- If you observe `AuthApiError` with `unexpected_failure` (500), check the DB trigger `handle_new_user()` and `public.migration_log` for details — see `db/migrations` for trigger migration files.
...
│   ├── KADA-Connect-be - Complete API Test Suite.postman_collection.json
...
│   │   ├── authController.js                 # Auth register/login/logout handlers
│   │   ├── companyController.js
│   │   ├── studentController.js
│   │   └── lookupController.js
│   ├── services/                             # Business logic and data operations
│   │   ├── authService.js                    # Supabase auth wrapper
│   │   ├── supabaseService.js                # Supabase client + token validation helper
...
│   │   ├── auth.js                           # Authentication middleware (requireAuth/optionalAuth)
...
├── db/
│   └── migrations/                           # SQL migrations and trigger scripts
├── working-tree.txt                           # Generated working tree (for sharing)
