...
const companySchemas = {
  ...
  update: Joi.object({
    ...
    preferredSkillsets: Joi.string().max(1000).allow(null, '').optional().messages({
      'string.max': 'Preferred skillsets cannot exceed 1000 characters'
    }),
    contactPersonName: Joi.string().min(1).max(100).allow(null, '').optional().messages({
      'string.min': 'Contact person name cannot be empty',
      'string.max': 'Contact person name cannot exceed 100 characters'
    }),
    contactEmailAddress: Joi.string().email().allow(null, '').optional().messages({
      'string.email': 'Contact email address must be a valid email'
    }),
    contactPhoneNumber: Joi.string().max(20).allow(null, '').optional().messages({
      'string.max': 'Contact phone number cannot exceed 20 characters'
    }),
...
const studentSchemas = {
  create: Joi.object({
    ...
    status: Joi.string().valid('Current Trainee', 'Alumni').required().messages({
